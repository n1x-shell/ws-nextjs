...
export default function AnaloguesPlayer() {
  ...
  const playWhenReady = useCallback((targetIdx: number) => {
    const attempt = (tries = 0) => {
      const muxEl = getMuxEl(wrapperRefs.current[targetIdx]);
      if (!muxEl) {
  ...
  const goToTrack = useCallback((newIndex: number, keepLyrics = false) => {
    ...
    setTimeout(() => {
      ...
      if (isAudioUnlocked()) {
        ...
        if (videoEl) audioEngine.connect(videoEl, ANALOGUES_TRACKS[n].index, ANALOGUES_TRACKS[n].displayTitle);
      }
    }, 200);
  }, [playWhenReady]);
    ...
    <div style={{
      ...
      {!audioUnlocked && (
        <PlayOverlay
          ...
          line1="⚠ ORGANIC SIGNAL DETECTED"
          line2="PRE-AUGMENTATION CONSCIOUSNESS TRACE"
          line3="ORIGIN: BIOLOGICAL // UNFILTERED"
        />
      )}

      {/* Persistent player pool */}
      {ANALOGUES_TRACKS.map((t, i) => (
        <div
          key={t.playbackId}
      ...
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
        ...
        {muted && (
          <div style={{
            ...
            opacity: 0.5, pointerEvents: 'none', fontFamily: 'inherit', letterSpacing: '0.05em',
          }}>MUTED · TAP</div>
        )}
        <div style={{
          position: 'absolute', top: '1rem', left: '0.75rem',
          fontSize: '0.65rem', color: 'var(--phosphor-green)',
          opacity: 0.5, pointerEvents: 'none', fontFamily: 'inherit', letterSpacing: '0.08em',
        }}>
