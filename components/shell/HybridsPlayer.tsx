...
function PlayOverlay({ onPlay }: { onPlay: () => void }) {
  ...
  const handle = useCallback(() => {
    ...
    setFading(true); onPlay(); setTimeout(() => setGone(true), 400);
  }, [fading, gone, onPlay]);
  ...
  return (
    ...
    >
      <div style={{
        ...
        border: '2px solid var(--phosphor-green)', display: 'flex',
        alignItems: 'center', justifyContent: 'center',
        boxShadow: '0 0 24px rgba(0,255,65,0.3)',
      }}>
        <div style={{
          width: 0, height: 0, borderTop: '1rem solid transparent',
          borderBottom: '1rem solid transparent',
          borderLeft: '1.6rem solid var(--phosphor-green)', marginLeft: '0.3rem',
        }} />
      </div>
    </div>
  );
...
interface TouchOrigin { x: number; y: number; time: number; zone: 'video' | 'panel'; }
...
function getVideoEl(muxEl: any): HTMLVideoElement | null {
  ...
  if (muxEl.shadowRoot) {
    const v = muxEl.shadowRoot.querySelector('video'); if (v) return v;
    const inner = muxEl.shadowRoot.querySelector('mux-player-theme, media-theme');
    const v2 = inner?.shadowRoot?.querySelector('video'); if (v2) return v2;
  }
  return muxEl.media?.nativeEl ?? muxEl.nativeEl ?? null;
}

// Off-screen preload: keeps hls.js buffering without display:none killing it.
...
export default function HybridsPlayer() {
  ...
  // ── Play active player, polling until hls has buffered ───────────────────

  const playWhenReady = useCallback((idx: number) => {
    const attempt = (tries = 0) => {
      const muxEl = getMuxEl(wrapperRefs.current[idx]);
      if (!muxEl) { if (tries < 20) setTimeout(() => attempt(tries + 1), 100); return; }
      muxEl.muted = false;
      if (muxEl.readyState >= 3) { muxEl.play?.().catch(() => {}); }
      else if (tries < 40) { setTimeout(() => attempt(tries + 1), 75); }
      else { muxEl.play?.().catch(() => {}); }
    };
    attempt();
  }, []);
  ...
  const handleGateUnlock = useCallback(() => {
    setAudioUnlocked(); lastMuteToggle.current = Date.now();
    setAudioUnlockedState(true); setMuted(false);
    audioEngine.setMuted(false); eventBus.emit('audio:user-gesture');
    playWhenReady(currentIndexRef.current);
  }, [playWhenReady]);

  useEffect(() => {
    const unsub = eventBus.on('audio:user-gesture', () => {
      setAudioUnlocked(); setAudioUnlockedState(true); setMuted(false);
      audioEngine.setMuted(false); audioEngine.resume();
    });
    return unsub;
  }, []);
  ...
  useEffect(() => {
    ...
    tryConnect(); const t = setTimeout(tryConnect, 400); return () => clearTimeout(t);
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent, zone: 'video' | 'panel') => {
    touchOrigin.current = {
      x: e.touches[0].clientX, y: e.touches[0].clientY, time: Date.now(), zone,
    };
  }, []);

  const handleTouchEnd = useCallback((e: React.TouchEvent) => {
    if (!touchOrigin.current) return;
    const o = touchOrigin.current; touchOrigin.current = null;
    ...
    if (o.zone === 'panel') { if (dy < 0) setLyricsOpen(true); else setLyricsOpen(false); }
    else { if (dy < 0) nextTrack(); else prevTrack(); }
  }, [nextTrack, prevTrack]);
  ...
  const handleVideoTap = useCallback(() => {
    ...
    setMuted(newMuted); audioEngine.setMuted(newMuted); audioEngine.resume();
  }, [muted]);
  ...
  return (
    <div style={{
      ...
      {/* ── Persistent player pool — all tracks always in the DOM ────────── */}
      {TRACKS.map((t, i) => (
        <div
          key={t.playbackId}
          ...
          style={i === currentIndex ? {
            ...
            opacity: transitioning ? 0 : 1, transition: 'opacity 0.2s ease',
          } : PRELOAD_STYLE}
        >
          <MuxPlayer
      ...
      {/* ── Overlay: touch + HUD + lyrics panel ──────────────────────────── */}
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
        ...
        {muted && (
          <div style={{
            ...
            fontSize: '0.65rem', color: 'var(--phosphor-green)', opacity: 0.5,
            pointerEvents: 'none', fontFamily: 'inherit', letterSpacing: '0.05em',
          }}>
            MUTED · TAP
          </div>
        )}
        ...
        <div style={{
          ...
          fontSize: '0.65rem', color: 'var(--phosphor-green)', opacity: 0.5,
          pointerEvents: 'none', fontFamily: 'inherit', letterSpacing: '0.08em',
        }}>
        ...
        <div style={{
          ...
          fontSize: '0.55rem', color: 'var(--phosphor-green)', opacity: 0.35,
          pointerEvents: 'none', fontFamily: 'inherit', letterSpacing: '0.12em',
        }}>
          HYBRIDS
        </div>
        ...
        {!lyricsOpen && (
          <div style={{
            ...
            flexDirection: 'column', gap: '0.35rem', opacity: 0.25,
            pointerEvents: 'none', fontSize: '0.7rem',
            color: 'var(--phosphor-green)', fontFamily: 'inherit',
          }}>
            <span>▲</span><span>▼</span>
          </div>
        )}
        ...
        >
          <div
            style={{
              flexShrink: 0, height: '3rem', display: 'flex',
          ...
          >
            ...
            <span style={{
              fontSize: '0.6rem', color: 'var(--phosphor-green)', opacity: 0.4,
              flexShrink: 0, fontFamily: 'inherit', letterSpacing: '0.08em',
              marginRight: '0.5rem',
            }}>
              {String(track.index).padStart(2, '0')}/{String(TRACKS.length).padStart(2, '0')}
            </span>
            <span style={{
              ...
              flexShrink: 0, userSelect: 'none', display: 'inline-block',
          ...
          >
            {track.lyrics ? (
              ...
              <div style={{
                fontFamily: 'inherit', fontSize: 'var(--text-base)',
                color: 'var(--phosphor-green)', opacity: 0.3,
                lineHeight: 1.8, fontStyle: 'italic', marginTop: '0.5rem',
              }}>
