...
export default function PixiCRT({ glitchIntensity }: PixiCRTProps) {
  ...
  useEffect(() => {
    ...
    // Initialize PixiJS with iOS-compatible settings
    const initPixi = async () => {
      ...
      try {
        ...
        await app.init({
          ...
          backgroundAlpha: 0,
          antialias: false, // Disable for better iOS performance
          resolution: Math.min(window.devicePixelRatio || 1, 2), // Cap at 2x for iOS
          autoDensity: true,
          preference: 'webgl', // Prefer WebGL but fallback to WebGPU
          powerPreference: 'low-power', // Better for mobile
        });

        // CRT Shader with iOS-compatible GLSL
        ...
        // Create filter with iOS-compatible settings
